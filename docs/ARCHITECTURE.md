...
- Fighter and Cleric transient class visuals now accept `effectScene`-only game-engine contexts instead of bailing out just because the entity scene is absent. That keeps multiplayer/alternate runtime paths on the same effect lane.
- Most important next architectural step: finish burning down any remaining direct `gameEngine.scene` gameplay visuals and class-specific fallback paths so transient combat readability no longer depends on ad hoc scene writes.
