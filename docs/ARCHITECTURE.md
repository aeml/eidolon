...
- Most important next architectural step: finish burning down the remaining direct `gameEngine.scene` gameplay visuals in the rest of Rogue and any leftover projectile/utility fallbacks so transient combat readability no longer depends on ad hoc scene writes.
