...
Playwright runs system Chrome at `/usr/bin/google-chrome` in the Codex environment and pinned Playwright Chromium in CI. Gameplay is driven through real DOM, keyboard, and mouse input. `page.evaluate()` is limited to read-only state inspection and Three.js projection used to position real mouse clicks. Reconnect faults are requested through the visible chat UI and performed by the allowlisted server-side `/qa-disconnect` command, outside page code. Live checks allow bounded recovery from transient edge 5xx responses, but require a complete client-module boot marker and retain the final failure when recovery never succeeds.
