...
- Current in-game displayed version: `Alpha 0.41.0.35`
- Active implementation line: `0.41` (procedural dark-fantasy cutover complete)
- Release-confidence gate: self-hosted locked browser dependencies, real-browser QA, QA-command restrictions, and deploy SHA/readiness verification gate the exact commit of every production release
...
- Verification at this checkpoint: the client and all Go packages pass their automated baselines; anonymous and disposable full-character Playwright routes run in hardware-accelerated system Chrome; production verification covers anonymous, persistent-character, extended gameplay/persistence, four-class animation, all-realm hazards, and two-account multiplayer against the exact deployed workflow SHA
...
- `0.40` (in progress): architecture decomposition — `entity.go` exists, but current `world.go` measures 8,578 physical lines; use current measurements, not the historical 7,444-line checkpoint, for completion gates
...
- `0.40.0`: extracted `server/internal/game/entity.go` (1,033 current lines) — entity model and related helpers. Historical notes recorded `world.go` at 7,444 immediately after extraction; the current file is 8,578 lines. Use the current release-gate evidence above rather than the historical test count.
