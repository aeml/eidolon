...
describe('UIManager asset download settings', () => {
    ...
    test('reflects real cache inspection results in status labels', async () => {
        ...
        ui.assetCacheManager.inspectPack = jest.fn(async (packName) => ({
            ...
            cachedVersion: packName !== 'environment-textures' ? '2026-09-03-26' : 'legacy-build'
        }));
        ...
        expect(document.getElementById('asset-pack-core-version').textContent).toContain('2026-09-03-26');
        expect(document.getElementById('asset-pack-dungeon-version').textContent).toContain('2026-09-03-26');
    ...
    test('successful downloads update last synced asset version', async () => {
        ...
        expect(ui.assetLastSyncedVersion.textContent).toContain('2026-09-03-26');
        expect(localStorage.getItem('eidolon.assetLastSyncedVersion')).toBe('2026-09-03-26');
    });
});
