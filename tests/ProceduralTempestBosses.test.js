...
import {
    PROCEDURAL_TEMPEST_BOSS_DEFINITIONS,
    PROCEDURAL_TEMPEST_BOSS_STATES,
    createProceduralRocMatriarch,
    createProceduralStormcallers,
    createProceduralTempestBoss,
    createProceduralThunderlordKaelix,
    createProceduralWindshear,
    createProceduralZephyrion,
    getProceduralTempestBossCacheMetrics
} from '../src/art/ProceduralTempestBosses.js';
import { Windshear } from '../src/entities/Windshear.js';
import { Stormcallers } from '../src/entities/Stormcallers.js';
import { RocMatriarch } from '../src/entities/RocMatriarch.js';
import { ThunderlordKaelix } from '../src/entities/ThunderlordKaelix.js';
import { Zephyrion } from '../src/entities/Zephyrion.js';

const CASES = Object.freeze([
    ['Windshear', createProceduralWindshear, Windshear, 65],
    ['Stormcallers', createProceduralStormcallers, Stormcallers, 70],
    ['RocMatriarch', createProceduralRocMatriarch, RocMatriarch, 70],
    ['ThunderlordKaelix', createProceduralThunderlordKaelix, ThunderlordKaelix, 80],
    ['Zephyrion', createProceduralZephyrion, Zephyrion, 95]
]);
...
describe('procedural Tempest Spire boss family', () => {
    ...
    test.each(CASES)('%s has five finite and articulated boss states', (type, create) => {
        ...
        expect(Object.keys(clips)).toEqual(PROCEDURAL_TEMPEST_BOSS_STATES);
        PROCEDURAL_TEMPEST_BOSS_STATES.forEach((state) => expect(clips[state].tracks.length).toBeGreaterThanOrEqual(9));
        ...
        for (const state of PROCEDURAL_TEMPEST_BOSS_STATES) {
    ...
    test('routing is explicit and all regional resources remain cached', () => {
        for (const [type] of CASES) expect(createProceduralTempestBoss(type).userData.proceduralActorType).toBe(type);
        expect(() => createProceduralTempestBoss('UnknownBoss')).toThrow('Unknown procedural Tempest Spire boss');
        const metrics = getProceduralTempestBossCacheMetrics();
        for (const [type] of CASES) createProceduralTempestBoss(type);
        expect(getProceduralTempestBossCacheMetrics()).toEqual(metrics);
        expect(metrics.geometries).toBeGreaterThanOrEqual(110);
