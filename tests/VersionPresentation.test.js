...
describe('version presentation', () => {
    test('advances the login screen to alpha 0.41.0.19 for the Molten Core boss procession', () => {
        expect(indexHtml).toContain('Alpha 0.41.0.19');
        expect(indexHtml).toContain('Patch 0.41.0.19');
        expect(indexHtml).toContain('five hearts beneath the anvil');
        expect(indexHtml).toContain('The Furnace Below has found its monsters');
        expect(indexHtml).toContain('Cindermaw runs like the beast it is');
        expect(indexHtml).toContain('The Scorched Twins share one terrible oath');
        expect(indexHtml).toContain('Pyrax brings the forge into battle');
        expect(indexHtml).toContain('The Guardian is a wall of black glass');
        expect(indexHtml).toContain('Infernax finally wears the Ashen Throne');
        expect(indexHtml).toContain('Large bodies no longer enlarge the rules');
        expect(indexHtml).toContain('2026-09-03-19');
    });
    ...
    test('keeps client, server, container, deploy, and isolated-QA version defaults aligned', () => {
        const expectedVersion = 'Alpha 0.41.0.19';
    ...
    test('marks 0.41.0.19 current and points the active line at 0.41', () => {
        expect(alphaRoadmap).toContain('Current in-game displayed version: `Alpha 0.41.0.19`');
