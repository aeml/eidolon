...
test('both uninterrupted story combat loops use ordinary retained-target input', () => {
    for (const [file, helper] of [['fresh-opening-gameplay.spec.js', 'selectCollectionTargetThroughInput'],
        ['fresh-story-hunt-route.js', 'selectEarnedAttackTarget']]) {
        const source = readFileSync(new URL(`./e2e/${file}`, import.meta.url), 'utf8');
        expect(source).toContain(`await ${helper}(page,`);
    ...
    expect(acquisition).toContain('await moveByGroundClick(page, dx * scale, dz * scale, {');
    expect(acquisition).toContain('moveOnly: true, minimumDistance:');
