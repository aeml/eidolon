...
describe('Combat telegraph escalation UI', () => {
    test('shows a boss warning callout with title, timing, and tone styling', () => {
        buildDom();
        const ui = new UIManager(false);

        ui.showCombatCallout({
        ...
        const panel = document.getElementById('combat-intent-panel');
        expect(panel.style.display).toBe('block');
        expect(document.getElementById('combat-intent-name').textContent).toBe('MAELSTROM SLAM');
        expect(document.getElementById('combat-intent-status').textContent).toContain('Brace for impact');
        expect(document.getElementById('combat-intent-meta').textContent).toContain('Incoming in 2.4s');
        expect(panel.dataset.calloutTone).toBe('boss');
    });
});
