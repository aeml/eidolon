...
import { installDeathSpiralAreaObserver } from './death-spiral-area-observer.js';

test.use({ viewport: { width: 390, height: 844 }, hasTouch: true, isMobile: true,
    userAgent: devices['Pixel 7'].userAgent, actionTimeout: 12_000,
    trace: 'off', screenshot: 'off', video: 'off' });

test('Death Spiral Fine Motor purchases change the paid footprint and persist through login', async ({ page, baseURL }, testInfo) => {
    test.setTimeout(240_000);
    test.skip(process.env.EIDOLON_E2E_DEATH_SPIRAL_AREA !== '1', 'Explicit isolated Death Spiral training route');
    ...
    await expect.poll(() => page.evaluate(() => window.game.player.hotbar.includes('Death Spiral'))).toBe(true);
    ...
    async function cast(rank, quality) {
        ...
        await page.evaluate(installDeathSpiralAreaObserver);
        const before = await page.evaluate(() => {
            const p = window.game.player;
            return { rank: p.talentRanks?.ROG_34 || 0, mana: p.stats.mana,
                x: p.position.x, z: p.position.z, slot: p.hotbar.indexOf('Death Spiral') };
        });
        ...
        await expect.poll(() => page.evaluate(() => window.__deathSpiralAreaNative.results.length)).toBe(1);
        await expect.poll(() => page.evaluate(() => window.__deathSpiralAreaNative.casts.length)).toBe(1);
        const observation = await page.evaluate(() => window.__deathSpiralAreaNative);
        expect(observation.results[0].accepted).toBe(true);
        expect(before.mana - observation.results[0].mana).toBe(35);
        const accepted = observation.casts[0], radius = 4 * (1 + .03 * rank);
        ...
        await page.screenshot({ path: testInfo.outputPath(`death-spiral-rank${rank}-${quality}.png`) });
        console.log(`[death-spiral-area] rank${rank}/${quality}: paid radius and attached boundary ${radius}`);
    }
    await cast(0, 'high');
    ...
    await cast(5, 'high');
    await testInfo.attach('death-spiral-trained-paid-casts', { body: JSON.stringify(receipts), contentType: 'application/json' });
