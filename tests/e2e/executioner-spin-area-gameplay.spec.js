...
test('phone Executioner Spin area purchases reach the authoritative ring and survive login', async ({ page, baseURL }, testInfo) => {
    ...
    await expect.poll(() => page.evaluate(() => window.game.player.hotbar.indexOf('Executioner Spin'))).toBeGreaterThanOrEqual(0);
    ...
    async function verifyCast(ranks, radius, quality) {
        ...
        await page.evaluate(() => {
            window.__spinArea = { casts: [], results: [] };
            if (window.__spinAreaObserver) return;
            window.__spinAreaObserver = true;
            const game = window.game, receive = game.handleServerMessage.bind(game);
            game.handleServerMessage = message => {
                const result = receive(message);
                if (message.type === 'ability_result' && message.payload?.skillName === 'Executioner Spin') window.__spinArea.results.push(message.payload);
                if (message.type === 'ability' && message.payload?.sourceId === game.player.id && message.payload.skillName === 'Executioner Spin') {
                    const effect = game.effects.find(effect => effect.isActive && effect.abilityShape?.sourceId === game.player.id && effect.abilityShape?.skillName === 'Executioner Spin');
                    const root = effect?.meshes?.[0];
                    const boundary = root?.children.find(part => part.userData.normalizedGameplayRadius === 1);
                    window.__spinArea.casts.push({ radius: message.payload.radius, arc: message.payload.arc,
        ...
        const state = await page.evaluate(() => ({ mana: window.game.player.stats.mana,
            ranks: ['FTR_24', 'FTR_33', 'FTR_38'].map(id => window.game.player.talentRanks?.[id] || 0),
            slot: window.game.player.hotbar.indexOf('Executioner Spin') }));
        expect(state.ranks).toEqual(ranks);
        await page.locator('#hotbar-container .hotbar-slot').nth(state.slot).tap();
        await expect.poll(() => page.evaluate(() => window.__spinArea.casts.length)).toBe(1);
        await expect.poll(() => page.evaluate(() => window.__spinArea.results.length)).toBe(1);
        const observation = await page.evaluate(() => window.__spinArea);
        expect(observation.results[0].accepted).toBe(true);
        expect(state.mana - observation.results[0].mana).toBe(40);
        ...
        console.log(`[executioner-spin-area] ranks ${ranks.join('/')}, ${quality}: accepted radius ${radius} matches attached ring`);
    }
    ...
    await verifyCast([0, 0, 0], 6, 'high');
    await buyFive('FTR_24'); await verifyCast([5, 0, 0], 6.6, 'low');
    await buyFive('FTR_33'); await verifyCast([5, 5, 0], 7.5, 'low');
    await buyFive('FTR_38'); await verifyCast([5, 5, 5], 8.1, 'low');
    await loginAndEnterWorld(page, credentials);
    await page.setViewportSize({ width: 844, height: 390 });
    await verifyCast([5, 5, 5], 8.1, 'high');
