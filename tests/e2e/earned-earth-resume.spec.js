...
test('continue the exact earned Earth save through its remaining four kills and manual handoff', async ({ page, baseURL }, testInfo) => {
    ...
    expect((await readChronicleChapter(page, hunt)).count).toBe(46);
    await earnFreshStoryHunt(page, credentials, hunt, { resumeAccepted: true, leaveTown: async () => {
    ...
    expect((await readChronicleChapter(page, EARTH_DUNGEON_CHAPTER)).accepted).toBe(false);
