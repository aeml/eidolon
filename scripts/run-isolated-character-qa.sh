...
docker build \
  ...
  --build-arg "BUILD_VERSION=Alpha 1.0.24" \
...
case "${EIDOLON_ISOLATED_QA_ROUTE:-all}" in
  ...
  phone-adventure)
    ...
    ;;
  *)
    echo "EIDOLON_ISOLATED_QA_ROUTE must be all, animations, multiplayer, movement, smoke, quests, inventory, extended, portal, dungeons, verdant, dungeon-full, chronicle-earth, chronicle-collection, fresh-opening, dungeon-recovery, direct-skills, projectile-walls, movement-walls, ground-walls, beam-walls, whirlwind, phone, phone-combat, phone-inventory, phone-quests, phone-build, phone-settings, or phone-adventure." >&2
    exit 1
    ;;
esac
qa_status=$?
