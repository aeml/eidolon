...
capture_party_qa_checkpoint() {
  if [[ ( "${EIDOLON_ISOLATED_QA_ROUTE:-all}" != party-dungeon && "${EIDOLON_ISOLATED_QA_ROUTE:-all}" != party-raid && "${EIDOLON_ISOLATED_QA_ROUTE:-all}" != earned-earth-resume && "${EIDOLON_ISOLATED_QA_ROUTE:-all}" != earned-water-resume && "${EIDOLON_ISOLATED_QA_ROUTE:-all}" != earned-party-dungeon ) || "${api_created}" != true || "${party_checkpoint_attempted}" == true ]]; then
    return 0
  fi
...
docker build \
  ...
  --build-arg "BUILD_VERSION=Alpha 1.9.22" \
...
run_rogue_utility() {
  EIDOLON_E2E_USERNAME="${QA_USERNAME_BASE}-rogue-utility" EIDOLON_E2E_CLASS=Rogue \
    EIDOLON_E2E_ROGUE_UTILITY=1 npx playwright test --retries=0 tests/e2e/rogue-utility-mastery-gameplay.spec.js
}

run_rogue_utility_techniques() {
  EIDOLON_E2E_USERNAME="${QA_USERNAME_BASE}-rogue-techniques" EIDOLON_E2E_CLASS=Rogue \
...
run_blade_storm_area() {
  EIDOLON_E2E_USERNAME="${QA_USERNAME_BASE}-blade-storm-area" EIDOLON_E2E_CLASS=Rogue \
...
case "${EIDOLON_ISOLATED_QA_ROUTE:-all}" in
  ...
  all)
    ...
    run_qa_stage forge-socket-appearance run_forge_socket_appearance
    ;;
  animations)
    run_animation_classes
    ;;
  ...
  portal)
    ...
    ;;
  casino-zone)
    EIDOLON_E2E_CASINO_MONGO_CONTAINER="${MONGO_CONTAINER}" EIDOLON_E2E_CASINO_MONGO_PORT="${mongo_port}" \
  ...
  party-guide-reentry)
    ...
    ;;
  earned-earth-resume)
    EIDOLON_E2E_EARNED_RESUME=1 EIDOLON_E2E_CLASS=Wizard EIDOLON_E2E_REST_RECOVERY=1 \
      EIDOLON_E2E_BUILD_MONGO_CONTAINER="${MONGO_CONTAINER}" EIDOLON_E2E_BUILD_MONGO_PORT="${mongo_port}" \
      npx playwright test --retries=0 tests/e2e/earned-earth-resume.spec.js
    ;;
  earned-water-resume)
    EIDOLON_E2E_EARNED_RESUME=1 EIDOLON_E2E_EARNED_WATER=1 EIDOLON_E2E_CLASS=Wizard EIDOLON_E2E_REST_RECOVERY=1 \
      EIDOLON_E2E_BUILD_MONGO_CONTAINER="${MONGO_CONTAINER}" EIDOLON_E2E_BUILD_MONGO_PORT="${mongo_port}" \
      npx playwright test --retries=0 tests/e2e/earned-water-resume.spec.js
    ;;
  recorded-build-dungeon)
    EIDOLON_E2E_EARNED_BUILD_DIAGNOSTIC=1 EIDOLON_E2E_CLASS=Wizard \
      EIDOLON_E2E_BUILD_MONGO_CONTAINER="${MONGO_CONTAINER}" EIDOLON_E2E_BUILD_MONGO_PORT="${mongo_port}" \
      npx playwright test --retries=0 tests/e2e/recorded-build-dungeon.spec.js
    ;;
