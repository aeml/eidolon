...
export function applyOfflineEarthshaker(source, aim, engine, isFriendlyActor) {
    ...
    const impact = (radius, amount, duration, line) => {
        ...
        for (const target of new Set(engine.chunkManager.getActiveEntities())) {
            ...
            const hit = line ? forward >= 0 && forward <= radius + body && lateral <= 1.5 + body
    ...
    impact(6, damage, stun, rune === 'earthshaker_fissure');
    if (rune === 'earthshaker_aftershock') {
        source.scheduleTask(() => {
            ...
            engine.spawnTransientEffect?.('wave', origin, 0xff8800, { source, abilityName: 'Earthshaker', radius: 3.5 });
            impact(3.5, Math.floor(damage / 2), aftershockStun, false);
        }, 1000);
    }
}
