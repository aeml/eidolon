...
func TestHandleMessageLevelCommandSetsPlayerLevelAndResponds(t *testing.T) {
	...
	if updated.BaseStats.Strength != 68 || updated.BaseStats.Vitality != 68 {
		t.Fatalf("expected level command to scale ordinary strength/vitality to 68/68 at level 30, got str=%d vit=%d", updated.BaseStats.Strength, updated.BaseStats.Vitality)
	}
	if updated.Damage <= 2 {
