...
func TestCasinoSaveProjectsWalkableExitWithoutRestoringResources(t *testing.T) {
	...
	p := &game.Entity{ID: "casino-save", Type: game.TypePlayer, State: "IDLE", X: position.ExitX, Z: position.ExitZ, Health: 37, MaxHealth: 100, Mana: 11, MaxMana: 50, WellRestedSeconds: 23}
	w.AddEntity(p)
	...
	if char.X != position.ExitX || char.Z != position.ExitZ || char.Y != 0 || char.InstanceID != "" {
