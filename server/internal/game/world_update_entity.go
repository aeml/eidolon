...
func (w *World) updateEntity(e *Entity, dt float64, players []*Entity, deferred *deferredActions) {
	...
	if e.Type == TypeEnemy {
		...
		if threatPlayer != nil {
			target = threatPlayer
			minDist = threatDist
			// Reduced starter awareness is not permission to damage enemies
			// from outside their response range without retaliation.
			sightRange = EnemySightRange
		} else {
		...
		if target != nil && minDist <= sightRange {
			...
			// Roam
			// Return at ordinary speed without healing, immunity or teleporting.
